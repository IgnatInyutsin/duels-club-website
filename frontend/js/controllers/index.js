main.controller('index',function($scope,$http,$location,$cookies){
	...
	//создаем объект для сбора списка новичков
	let newers = new DataCollector ({
		$scope: $scope,
		$cookies: $cookies,
	...
	let login = new Button ({
		...
		id: "loginButton",
		$cookies: $cookies,
		clickFunction: function () { //при нажатии на кнопку сработает эта функция
			...
			let nickname = $('input#nickname').val(); //собираем данные
			...
			$scope.login.joinWithBackend({
				...
				additionalFunc: function (responseData) {
					...
					if (responseData == undefined) { //если нет такого пользователя
	...
	let emptyLoginVal = new Checkup({
		...
		ifTrue: function() {login.disabled()},
		ifFalse: function() {login.enabled()}
	});
